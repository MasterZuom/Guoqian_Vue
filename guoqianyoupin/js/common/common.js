...
function resource(config) {
    ...
    // config.apiBaseUrl = 'https://www.gqall.com//h5shop/api'
...
if(getParam('jkcardtoken')){//一卡通过来用户判断
    ...
    resource({
        type: 'get',
        api: '/weixin/jkcard_wxLogin?mobile='+ getParam('mobile') +'&jkcardtoken='+ getParam('jkcardtoken')
    })
...
//                 // window.location.href='https://open.weixin.qq.com/connect/oauth2/authorize?appid='+ data.appid +'&redirect_uri='+localHref+ '&response_type='+ data.response_type +'&scope='+ data.scope +'&state=STATE#wechat_redirect'
//                 window.location.href='https://open.weixin.qq.com/connect/oauth2/authorize?appid='+ data.appid +'&redirect_uri='+localHost+'/turn.html' + '&response_type='+ data.response_type +'&scope='+ data.scope +'&state=STATE#wechat_redirect'
//             }
//
//         })
// }
...
// 清除缓存中的除uniqueId以外的信息
function clearSomeUserInfo() {
    var uniqueId = JSON.parse(window.localStorage.getItem('loginInfo')).uniqueId;
    var loginInfo = {
        uniqueId:uniqueId
    }
